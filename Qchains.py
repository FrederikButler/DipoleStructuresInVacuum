...
def StructurePlotter(points_3d, title="Geometry of the atoms"):
    ...
    fig = go.Figure(
        data=[
            go.Scatter3d(
                x=x, y=y, z=z, mode="markers", marker=dict(size=5, opacity=0.8)
            )
        ]
    )
...
def FindEigenstates(points, dipole_vec_hat, distance):
    ...
    r_ij_hat = np.divide(
        r_ij_vector,
        r_ij,
        out=np.zeros_like(r_ij_vector, dtype=float),
        where=(r_ij != 0),
    )
    ...
    for l in range(len(k_e__r_ij)):
        ...
        f_ji = 3 / 2 * (1 - (dot_product) ** 2) * np.sin(
            k_e__r_ij[l].flatten()
        ) / safe_denom1 + 3 / 2 * (1 - 3 * (dot_product) ** 2) * (
            np.cos(k_e__r_ij[l].flatten()) / safe_denom2
            - np.sin(k_e__r_ij[l].flatten()) / safe_denom3
        )
        g_ji = -3 / 2 * (1 - (dot_product) ** 2) * np.cos(
            k_e__r_ij[l].flatten()
        ) / safe_denom1 + 3 / 2 * (1 - 3 * (dot_product) ** 2) * (
            np.sin(k_e__r_ij[l].flatten()) / safe_denom2
            + np.cos(k_e__r_ij[l].flatten()) / safe_denom3
        )
...
def PlotEigenstateEvolution(
    eigenvalues,
    eigenvectors,
    time,
    title="Time evolution starting in eigenstate",
    legend=True,
):
    """
    ...
    plt.plot(
        time, np.exp(-time), c="red", linestyle="--", label=r"$e^{-t \ \Gamma_0}$"
    )  # for better distinction of sub and super radiant states
    if legend:
        plt.legend()
    plt.title(title)
...
def RandInitialStateEvolution(
    eigenvalues, eigenvectors, init_state, time, return_init_lambdabase=False
):
    """
    ...
    """
    A_eigenvectors_inv = np.linalg.inv(
        eigenvectors
    )  # so that A_eigvectors_inv * eigenvectors(matrix) = I
    ...
    coeff_evolution_lambdabase = init_state_lambdabase[:, np.newaxis] * np.exp(
        eigenvalues[:, np.newaxis] * time
    )
    coeff_evolution_ibase = eigenvectors @ coeff_evolution_lambdabase
    prob_exc_sites_t = (
        coeff_evolution_ibase * coeff_evolution_ibase.conj()
    ).real  # is already real but due numerical inaccuracies we get like e-17. So just to get rid of these
...
def SiteProbDistribution(eig_values, eig_vectors, state_num, figsize, hue=False):
    ...
    sorted_eig_values = eig_values[sorted_indices]  # noqa: F841
    ...
    site_prob = (
        state_vec * state_vec.conj()
    ).real  # only take real to get rid of the numerical imprecision
    ...
    if hue:
        ...
        cyclic_two_color = LinearSegmentedColormap.from_list(
            "cyclic_two", ["blue", "red", "blue"]
        )
        norm = Normalize(vmin=-np.pi, vmax=np.pi)
    ...
    bars = ax.bar(sites, site_prob, color=colors, label=r"$\mid c_j \mid^2$")  # noqa: F841
...
def SiteAmplitudeDistribution(
    eig_values, eig_vectors, state_num, hue=False, figsize=(7, 5)
):
    # sort eigenvalues and eigenvectors. Lowest index being most sub-radiant and highest subradiant
    sorted_indices = np.lexsort((eig_values.real, np.abs(eig_values.real)))
    sorted_eig_values = eig_values[sorted_indices]  # noqa: F841
    ...
    site_coeff = (
        state_vec.real
    )  # only take real to get rid of the numerical imprecision
...
def SiteProbAndAmplitudeDistribution(
    eig_values,
    eig_vectors,
    state_num,
    titleprecision,
    hue=True,
    sharey=True,
    interrupted_array=False,
    is_ring=False,
    is_hedgehog=False,
    is_star=False,
    return_sorted_eigval=False,
):
    # sort eigenvalues and eigenvectors. Lowest index being most sub-radiant and highest subradiant
    sorted_indices = np.lexsort((eig_values.real, np.abs(eig_values.real)))
    ...
    ##The state we plot
    state_vec = sorted_eig_vectors[:, state_num]
    state_vec_decay_rate = np.round(
        -2 * sorted_eig_values[state_num].real, titleprecision
    )

    # Setup the plot
    if interrupted_array:
        sites = np.arange(
            0, len(eig_vectors) + 1, 1
        )  # one extra so we can plot central site with 0 amplitude
        ...
        site_coeff = np.concatenate(
            [site_coeff_temp[:missing_site], [0], site_coeff_temp[missing_site:]]
        )
        site_prob = np.concatenate(
            [site_prob_temp[:missing_site], [0], site_prob_temp[missing_site:]]
        )
    else:
        sites = np.arange(1, len(eig_vectors) + 1, 1)
        site_coeff = (
    ...
    bars_prob = axs[0].bar(
        sites, site_prob, color=colors
    )  # label = r'$ |c_j|^2 $'  # noqa: F841
    bars_site = axs[1].bar(
        sites, site_coeff, color=colors
    )  # label = r'$ c_j $'  # noqa: F841
    ...
    if is_star:
        axs[0].axvline(
            x=sites[len(sites) // 2] - 0.5,
            color="grey",
            linestyle="--",
            linewidth=0.5,
            label="Seperation of inner and outer ring",
        )
        axs[1].axvline(
            x=sites[len(sites) // 2] - 0.5,
            color="grey",
    ...
    if interrupted_array:
        fig.suptitle(
            rf"$\xi = {state_num + 1}$ for interrupted, $\Gamma / \Gamma_0 = {state_vec_decay_rate}$",
            x=0.43,
            fontsize=fontsize + 2,
        )
    elif is_ring:
        fig.suptitle(
            rf"$\xi = {state_num + 1}$ for ring, $\Gamma / \Gamma_0 = {state_vec_decay_rate}$",
            x=0.43,
            fontsize=fontsize + 2,
        )
    elif is_hedgehog:
        fig.suptitle(
            rf"$\xi = {state_num + 1}$ for hedgehog, $\Gamma / \Gamma_0 = {state_vec_decay_rate}$",
            x=0.43,
            fontsize=fontsize + 2,
        )
    elif is_star:
        fig.suptitle(
            rf"$\xi = {state_num + 1}$ for star, $\Gamma / \Gamma_0 = {state_vec_decay_rate}$",
            x=0.43,
            fontsize=fontsize + 2,
        )
    else:
        fig.suptitle(
            rf"$\xi = {state_num + 1}$ for array, $\Gamma / \Gamma_0 = {state_vec_decay_rate}$",
            x=0.43,
            fontsize=fontsize + 2,
        )
    # plt.legend()
